...
import 'package:getrightmealnew/Controllers/AuthControllers/RegisterProfileInfo.dart';
import 'package:getrightmealnew/Controllers/OnBoardingControllers/Screen2Controller.dart';
import 'package:getrightmealnew/Models/UserProfileModel.dart';
...
class EditProfileScreen extends StatefulWidget {
  ...
  const EditProfileScreen({super.key,  this.userProfileModel});
...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  VerifyOtpScreenController verifyOtpScreenController = Get.put(VerifyOtpScreenController());

  RxBool gettingApi = false.obs;

Future<void> getApi ()async{
  try{

    gettingApi.value = true;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((v)async{
      ...
      // controller.resetAll();
      print("user profile model gotten at edit profile screen  : "+widget.userProfileModel!.toJson().toString());
      controller.nameController.value.text = widget.userProfileModel!.name!;
      try{
        controller.selectedDateOfBirth.value = widget.userProfileModel!.profile!.dateOfBirth!;
      }catch(e){

      }
      try{

        controller.cmInchFeet.value = widget.userProfileModel!.profile!.heightUnit.toString();
        controller.kgLbs.value = widget.userProfileModel!.profile!.weightUnit.toString();

      }catch(e){
        debugPrint("Could not pre-fill the height and weight units");
      }
      print("1");
      controller.cityController.value.text = widget.userProfileModel!.profile!.city!;
      print("2");
      controller.phoneController.value.text = widget.userProfileModel!.profile!.phone!;
      controller.phone.value = controller.phoneController.value.text;
      controller.city.value= controller.cityController.value.text;
      print("3");
      controller.selectedGender.value = widget.userProfileModel!.profile!.gender!;
      print("controller.selected gender: "+ controller.selectedGender.value);
      print("Controller.country.value : "+widget.userProfileModel!.profile!.country.toString());
      controller.country.value = widget.userProfileModel!.profile?.country!??1 ;
      print("4");
      controller.selectedHeight.value = double.parse(widget.userProfileModel!.profile!.height.toString());
      controller.heightController.value.text = controller.selectedHeight.toString();
      print("height: "+controller.selectedHeight.value.toString());
      print("5");
      // _scrollController = FixedExtentScrollController(initialItem: widget.userProfileModel!.profile.height <49? widget.userProfileModel!.profile.height-1 :widget.userProfileModel!.profile.height-50);
      // heightController.scrollToItem(widget.userProfileModel!.profile.height!-50, duration: Duration(milliseconds: 400), curve: Curves.easeInOut);
      // controller.selectedDateOfBirth.value = widget.userProfileModel!.profile.dateOfBirth!;
      print("6");

      controller.selectedGoal.value = (widget.userProfileModel!.profile!.goal)!;

      print("7");
      controller.selectedActivityLevel.value = (widget.userProfileModel!.profile!.activityLevel)!;

      print("8");
      controller.selectedWeight.value = widget.userProfileModel!.profile!.weight!.toInt();
       controller.weightController.value.text = controller.selectedWeight.toString();
      print("9");

      controller.selectedDietaryPreferences.value = widget.userProfileModel!.profile!.dairyPreferences!
          .map((e) => int.tryParse(e) ?? -1) // Convert string to int, handle errors
          .where((index) => index != -1) // Remove invalid indexes
          .toList();
      controller.selectedCuisinePreferences.value = widget.userProfileModel!.profile!.cuisinePreferences!
          .map((e) => int.tryParse(e) ?? -1) // Convert string to int, handle errors
          .where((index) => index != -1) // Remove invalid indexes
          .toList();

      print("10");
      print("selected dietary preferences: "+controller.selectedDietaryPreferences.toString());
    });
  }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
        ...
        child:Obx(()=> Scaffold(
          ...
          body: gettingApi.value? Center(child: Column(
            crossAxisAlignment: CrossAxisAlignment.center,
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              SizedBox(width: w,),
              MyText(
                title: 'Pre-filling your profile...',
                color: dustyRose,
                fontWeight: w600,
                size: size16,
              ),
                        ...
                        Padding(
                          ...
                          child: Obx(() {
                            ...
                            return AnimatedSwitcher(
                              ...
                              reverseDuration: Duration(milliseconds: 500), // Disable outgoing animation
                              transitionBuilder: (child, animation) {
                                return SlideTransition(
                                  position: Tween<Offset>(
                                    begin: Offset(0, -4), // New widget slides in from the top
                                    end: Offset.zero,
                                  ).animate(animation),
                      ...
                      button = InkWell(
                        onTap: (){
                          if (controller.selectedCuisinePreferences.isNotEmpty) {
                            Get.to(
                                  () => RegisterProfileInfo(fromRegister: false),
  ...
  Widget genderButton(Screen2Controller controller, {required String gender, required String image}) {
    ...
    return GestureDetector(
      ...
      child: Obx(() {
        bool isSelected = controller.selectedGender.value.toLowerCase() == gender.toLowerCase();
